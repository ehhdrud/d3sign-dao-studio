...
// 상태 변경이 발생하는 컴포넌트는 이 함수를 import하여 Context 객체의 값을 읽어들일 수 있다.
export function useHoveredIndex() {
  return useContext(HoveredIndexContext);
}

// Provider 내부에 지정된 컴포넌트에서 상태 변화가 일어나면 그 상태를 Context 객체에게 제공하는 역할을 하는 'HoveredIndexProvider' 함수 정의
export function HoveredIndexProvider({ children }) {
  const [hoveredIndex, setHoveredIndex] = useState(null);

  return (
    // Context 객체에 저장되는 value 값을 하위 컴포넌트에게 전달한다.
